...
- Fontlar depoda tek kopya: `apps/web/public/fonts/` (SIL OFL, lisans dosyaları yanında). `css/haze.css` oraya göreli yolla bağlanır.
...
- Gövde kalınlığı Michroma'nın "I" harfinden, gövde konumları ve çizgi yüksekliği Michroma "H" harfinden ölçülür.
...
- Hazır dosyalar: `apps/web/public/logo/` (yazı logosu 5 renk + 4 zeminli, ikon 4 renk, monogram 2 renk; varlık logoları `assets/` altında).
...
- Her iki yüzün alt şeridinde **yalnız Stellar işareti** (yazı yok), ortalı, gravür.

## 8. Mockup sahneleri
taş yüzeyde telefon + kılıftan çıkan kart, POS'a temassız ödeme, koyu deri zeminde yelpaze. CSS 3B (perspective + rotateX/rotateZ), her nesnenin altında bulanık gölge, kart kenarı `box-shadow: 0 3px 0 #8A6A28`.

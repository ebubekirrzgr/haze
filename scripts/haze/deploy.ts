...
import { DEMO_RESERVES } from "@haze/stellar";
...
const prices: Record<string, string> = { USDC: "10000000", hUSDY: "10000000", hXAU: String(Math.round(Number(process.env.XAU_USD ?? 2400) * 1e7)) };
const assetsJson = JSON.stringify((["USDC", "hUSDY", "hXAU"] as const).map((c) => ({ Stellar: cfg.assets[c].sac })));
const pricesJson = JSON.stringify((["USDC", "hUSDY", "hXAU"] as const).map((c) => prices[c]));
...
if (!cfg.haze.hazeCredit) {
  ...
  for (const c of ["USDC", "hUSDY", "hXAU"] as const) {

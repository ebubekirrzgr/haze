 ...
 */
export type AssetCode = "USDC" | "hUSDY" | "hXAU" | "hNVDA" | "hSHEL" | "hBMW" | "hTRY";
/** Havuzda teminat olabilen varlıklar (hTRY yalnızca giriş/çıkış bacağı). */
export type CollateralCode = Exclude<AssetCode, "hTRY">;
...
export const ASSET_META: Record<AssetCode, AssetMeta> = {
  ...
  hTRY: { code: "hTRY", name: "Türk lirası", kind: "fiat", mainnetRef: "TRY stablecoin", baseUsd: 0, displayDecimals: 2, mint: "50000000", demoAmount: "0", mmSize: "100000" },
};
...
export const ISSUED_CODES: Exclude<AssetCode, "USDC">[] = ["hUSDY", "hXAU", "hNVDA", "hSHEL", "hBMW", "hTRY"];
...
export const DEMO_RESERVES: Record<CollateralCode, { c_factor: number; l_factor: number; util: number; max_util: number }> = Object.fromEntries(
  COLLATERAL_CODES.map((c) => [c, ASSET_META[c].reserve!]),
) as Record<CollateralCode, { c_factor: number; l_factor: number; util: number; max_util: number }>;

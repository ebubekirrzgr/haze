...
const STR = {
  ...
  "kazan.ekleNot": { tr: "Tek işlem: USDC kasana geçer ve teminat olur. Ağ ücretini HAZE karşılar.", en: "One transaction: USDC moves to your vault and becomes collateral. HAZE pays the network fee.", pt: "Uma transação: o USDC vai para o seu cofre e vira garantia. A HAZE paga a taxa de rede.", es: "Una transacción: el USDC pasa a tu bóveda y se convierte en garantía. HAZE paga la comisión de red." },
  ...
  "kazan.dagitNot": { tr: "Tek passkey onayı: payı olan her varlık DEX'te alınır ve kasana yatırılır; ağ ücretlerini HAZE karşılar.", en: "One passkey approval: every asset with a share is bought on the DEX and deposited into your vault; HAZE pays the network fees.", pt: "Uma aprovação com passkey: cada ativo com participação é comprado na DEX e depositado no seu cofre; a HAZE paga as taxas de rede.", es: "Una aprobación con passkey: cada activo con participación se compra en la DEX y se deposita en tu bóveda; HAZE paga las comisiones de red." },
  "kazan.dagitBtn": { tr: "Dağılımı uygula", en: "Apply allocation", pt: "Aplicar alocação", es: "Aplicar asignación" },
  "kazan.cekNot": { tr: "Teminattan cüzdanına çekilir. Sağlık faktörü bozulacaksa havuz işlemi reddeder.", en: "Withdrawn from collateral to your wallet. The pool rejects it if the health factor would break.", pt: "Sacado da garantia para a sua carteira. O pool recusa se o fator de saúde ficar comprometido.", es: "Se retira de la garantía a tu billetera. El pool lo rechaza si el factor de salud se rompe." },
  "kazan.cekBtn": { tr: "Kazan'dan çek", en: "Withdraw from Earn", pt: "Sacar do Rendimento", es: "Retirar de Rendimiento" },
  "kazan.adimDeposit": { tr: "Kasaya yatır (USDC → teminat)", en: "Deposit to vault (USDC → collateral)", pt: "Depositar no cofre (USDC → garantia)", es: "Depositar en la bóveda (USDC → garantía)" },
  "kazan.adimWithdraw": { tr: "Kasadan çek ({code} → cüzdan)", en: "Withdraw from vault ({code} → wallet)", pt: "Sacar do cofre ({code} → carteira)", es: "Retirar de la bóveda ({code} → billetera)" },
  // kart
  ...
  "kart.not": { tr: "Onay saniyenin altında, limit hesabıyla verilir; borç hemen ardından havuzda açılır. Türkiye'deki POS'ta TL ödenir, borç TL cinsinden tutulur.", en: "Authorization is decided in under a second from your credit limit; the loan opens in the pool right after. A POS in Türkiye charges TRY and the debt is kept in lira.", pt: "A autorização é decidida em menos de um segundo a partir do seu limite; o empréstimo abre no pool logo depois. Um POS na Turquia cobra em TRY e a dívida fica em liras.", es: "La autorización se decide en menos de un segundo a partir de tu límite; el préstamo se abre en el pool justo después. Un POS en Turquía cobra en TRY y la deuda se mantiene en liras." },
  "kart.ac": { tr: "Kartı aç", en: "Unfreeze card", pt: "Desbloquear cartão", es: "Desbloquear tarjeta" },
  "kart.dondur": { tr: "Kartı dondur", en: "Freeze card", pt: "Congelar cartão", es: "Congelar tarjeta" },
  ...
  "nakit.adimTeklif": { tr: "Kur teklifi ve çekim talimatı", en: "Rate quote and withdrawal order", pt: "Cotação e ordem de saque", es: "Cotización y orden de retiro" },
  ...
  "profil.isverenNot": { tr: "İşveren TL maaş gönderir; anchor kur teklifi verir ve USDC'ye çevirir; maaş günü borç kapanır, kalan kasana eklenir.", en: "The employer sends a TRY salary; the anchor quotes a rate and converts it to USDC; on payday the debt is repaid and the rest goes to your vault.", pt: "O empregador envia o salário em TRY; o anchor cota o câmbio e converte em USDC; no dia do pagamento a dívida é quitada e o restante vai para o seu cofre.", es: "El empleador envía el salario en TRY; el anchor cotiza el cambio y lo convierte a USDC; el día de pago se liquida la deuda y el resto va a tu bóveda." },
  "profil.maasYatir": { tr: "₺ Maaş yatır", en: "₺ Pay salary", pt: "₺ Pagar salário", es: "₺ Pagar salario" },
  "profil.maasGunu": { tr: "Maaş günü simülasyonu", en: "Simulate payday", pt: "Simular dia do pagamento", es: "Simular día de pago" },
  ...
  "kazan.adimBorrow": { tr: "Borç al ({code} → cüzdan)", en: "Borrow ({code} → wallet)", pt: "Emprestar ({code} → carteira)", es: "Pedir prestado ({code} → billetera)" },
  "kazan.adimSwap": { tr: "USDC → {code} (DEX)", en: "USDC → {code} (DEX)", pt: "USDC → {code} (DEX)", es: "USDC → {code} (DEX)" },
  "kazan.adimDepositN": { tr: "Kasaya yatır ×{n}", en: "Deposit to vault ×{n}", pt: "Depositar no cofre ×{n}", es: "Depositar en la bóveda ×{n}" },
  "nakit.adimBorc": { tr: "Kasadan borç al ({tutar})", en: "Borrow from vault ({tutar})", pt: "Emprestar do cofre ({tutar})", es: "Pedir prestado a la bóveda ({tutar})" },
  "nakit.adimCek": { tr: "Kasadan {code} çek ({tutar})", en: "Withdraw {code} from vault ({tutar})", pt: "Sacar {code} do cofre ({tutar})", es: "Retirar {code} de la bóveda ({tutar})" },
  "nakit.adimDex": { tr: "DEX'te USDC'ye çevir ve anchor'a gönder", en: "Convert to USDC on the DEX and send to the anchor", pt: "Converter em USDC na DEX e enviar ao anchor", es: "Convertir a USDC en la DEX y enviar al anchor" },
  "kazan.adimRepay": { tr: "Borcu öde ({code})", en: "Repay ({code})", pt: "Quitar ({code})", es: "Pagar ({code})" },
